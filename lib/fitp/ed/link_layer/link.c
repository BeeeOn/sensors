...
void PHY_process_packet(uint8_t* data,uint8_t len){
    ...
    {
        D_LINK printf("RX: GLOBAL_STORAGE.waiting_join_response: %d\n", GLOBAL_STORAGE.waiting_join_response);
...
bool LINK_send_join_request(uint8_t* payload, uint8_t payload_len){
	  DEBUG(ED, 17, "LINK_send_join_request()\n");
    D_LINK printf("LINK_send_join_request()\n");
    // staci uint8_t packet[20];??? -> otestovat
    uint8_t packet[63];
    // size of link header
    bool result;
    uint8_t my_channel;

   // cekam na ACK JOIN REQUEST
   // neni mozno odstranit, ED se paruje nejen jednou (duplicitni by to bylo jen v pripade, ze je JOIN REQUEST poslan hned po inicializaci)
   LINK_STORAGE.link_ack_join_received = false;
   // cekam na JOIN RESPONSE
   GLOBAL_STORAGE.waiting_join_response = true;

   // zjisteni aktualniho kanalu
   my_channel = PHY_get_channel();
   for(uint8_t i = 0; i <= MAX_CHANNEL; i++)
   {// postupne nastavovani kanalu
	  uint8_t packet_index = 10; 
      result = PHY_set_channel(i);
      if(result != false)
      {// overeni, zda se kanal podarilo nastavit
            gen_header(packet, LINK_DATA_TYPE, LINK_DATA_JOIN_REQUEST);
            for(uint8_t index = 0; index < payload_len; index++)
            {// prekopirovani dat ze sitove vrstvy
                 packet[packet_index++] = payload[index];
            }
  ...
  //if(!LINK_STORAGE.link_ack_join_received)
  //{// nikdo mi nikde neodpovi, nastavi se kanal, na kterem jsem byla pred scanovanim (nedoslo k prijeti ACK JOIN REQUEST)
      result = PHY_set_channel(my_channel);
      printf("Maybe default channel\n");
      return false;
  //}
  // ===========================================================================
