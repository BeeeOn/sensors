...
/*
 * Listen for noise on medium and the transmit data
 */
...
/*
 * IRQ when data are received or sent
 */
void HW_irq1occurred(void){
    if (PHY_STORAGE.mode == RF_RECEIVER) {
        do {  
            ...
            //HW_sniffRX(recived_len,(const char*) PHY_STORAGE.recived_packet+1);
            PHY_process_packet(PHY_STORAGE.recived_packet + 1 , recived_len - 1);
        }
        while (get_register(FTXRXIREG) & 0x02);  // read data until there are some        
