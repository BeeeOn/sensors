 ...
 */ 
bool add_device(uint8_t *edid, uint8_t cid, bool sleepy) {
    ...
    for (uint8_t i=0; i<MAX_DEVICES;i++) {
        if (GLOBAL_STORAGE.devices[i].valid) {
            fs << setfill('0') << hex
               ...
               << GLOBAL_STORAGE.devices[i].sleepy << endl;                 
        }
    }
    fs.close();
    if (!fs) return false;
...
bool load_device_table(void) {
    ...
    while (getline(fs, line)) {
        ...
        if (!(iss >> hex >> edid[0] >> edid[1] >> edid[2] >> edid[3] >> 
              delimiter >> cid >> delimiter >> sleepy)) continue;
        GLOBAL_STORAGE.devices[i].valid = true;
 ...
 */ 
void send_join_response(uint8_t *toed, uint8_t *payload, uint8_t len) {
    uint8_t tmp[MAX_NET_PAYLOAD_SIZE];
    uint8_t index=0;
    
    tmp[index++] = (PT_DATA_JOIN_RESPONSE << 4) & 0xF0;
    tmp[index++] = GLOBAL_STORAGE.cid & 0x3f;
    tmp[index++] = toed[0];
    tmp[index++] = toed[1];
    tmp[index++] = toed[2];
    tmp[index++] = toed[3];
...
void LINK_join_request_received(uint8_t signal_strenght, uint8_t* payload, uint8_t len) {
    ...
    if(type == PT_DATA_JOIN_REQUEST) {
        ...
        if (GLOBAL_STORAGE.pair_mode) {
            send_join_response(sedid, (uint8_t *) "", 0);
            add_device(sedid, 0, payload[1] == 0xFF);
